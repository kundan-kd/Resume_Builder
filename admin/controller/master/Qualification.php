...
$action = $_POST['action'];
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>
                        ...
                        <td>" . $row['name'] . "</td>
                        <td>
                        <button class='btn btn-outline-primary btn-sm me-2 edit-qualification-btn'
                                data-id='{$row['id']}' '>
                            <i class='ri-pencil-line'></i>
                        </button>
                        <button class='btn btn-outline-danger btn-sm delete-qualification-btn' 
                                data-id='{$row['id']}'>
    ...
    if($action == 'add'){
        ...
        if(mysqli_query($conn, $insert)){
            ...
            echo json_encode(['success' => 'Qualification not added']);
        }
    }
